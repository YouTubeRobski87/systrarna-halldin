...
export const load = async ({ url }) => {
	...
	const orders = (data as OrderRecord[]).filter((order) => {
		const matchesPayment = !paymentStatus || paymentStatus === 'all' || order.payment_status === paymentStatus;
		const matchesOrder = !orderStatus || orderStatus === 'all' || order.order_status === orderStatus;
		const haystack = [order.order_number, order.customer_name, order.customer_email, order.customer_phone].join(' ').toLocaleLowerCase('sv-SE');
		return matchesPayment && matchesOrder && (!search || haystack.includes(search));
	});
	return { orders, paymentStatuses, orderStatuses, filters: { paymentStatus: paymentStatus ?? 'all', orderStatus: orderStatus ?? 'all', search } };
};
