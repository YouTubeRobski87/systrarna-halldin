	...
	<form class="admin-filters" method="GET">
		<label
			>Sök<input
				...
				placeholder="Ordernummer, namn, mejl eller telefon"
			/></label
		>
		<label
			>Betalning<select name="paymentStatus" value={data.filters.paymentStatus}
				><option value="all">Alla</option>{#each data.paymentStatuses as status}<option
						value={status}>{paymentStatusLabels[status as keyof typeof paymentStatusLabels]}</option
					>{/each}</select
			></label
		>
		<label
			>Orderstatus<select name="orderStatus" value={data.filters.orderStatus}
				><option value="all">Alla</option>{#each data.orderStatuses as status}<option value={status}
						>{orderStatusLabels[status as keyof typeof orderStatusLabels]}</option
					>{/each}</select
			></label
		>
