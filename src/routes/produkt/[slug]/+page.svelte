...
<main class="page">
	...
	<section class="product-detail">
		<div class="detail-image"><span>{data.product.image}</span></div>
		<div class="detail-copy">
			<p class="eyebrow">{data.product.category}</p>
			<h1>{data.product.name}</h1>
			<strong class="price">{currency(data.product.price)}</strong>
			<p class:low-stock={data.product.stock < 4} class="stock">
			...
			{#if added}<p class="toast detail-toast">Tillagd i varukorgen ♡</p>{/if}<small
				>Handgjort eller utvalt med omsorg. Begränsat antal.</small
			>
		</div>
	</section>
